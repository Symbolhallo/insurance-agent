...
- `CONFIRMING` and `RESUMING` are leased transient states. Expired claims are released by `WorkflowLeaseRecoveryJob`; terminal states are protected from late failure overwrites.
- Every top-level request must provide a `requestId`. `(conversationId, requestId)` is unique, and `ai_conversation_workflow_lock` permits only one active top-level workflow per conversation.
