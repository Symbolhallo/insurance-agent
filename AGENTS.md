...
- The authoritative Main Workflow entry chain starts at `POST /api/v1/workflows/main/runs/stream`: subscribe before background execution, create the workflow transaction, resolve products before context alignment, optionally interrupt for confirmation, then run intent -> planner -> dynamic DAG -> summary -> output review -> transactional finalization. Keep code comments and `docs/project-understanding-guide.md` aligned to this order.
