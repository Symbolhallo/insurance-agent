...
官方仓库中曾有关于嵌套子图与 Append 状态重复合并的缺陷报告。因此必须通过集成测试验证当前 `1.1.2.0` 的实际行为，不应仅凭示例推断。
...
Codex 不得直接复制这一段，必须查看 `1.1.2.0` 的实际源码和测试。
